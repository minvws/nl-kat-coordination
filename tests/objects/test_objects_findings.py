...
def test_ooi_finding_list(rf, client_member, tree):
...
def test_mute_finding_button_is_visible(request, member, rf, octopoes_api_connector, tree):
...
def test_mute_finding_button_is_not_visible_without_perms(request, member, rf, octopoes_api_connector, tree):
    member = request.getfixturevalue(member)
...
@pytest.mark.parametrize("member", ["superuser_member", "redteam_member"])
def test_mute_finding_form_view(request, member, rf, octopoes_api_connector):
    member = request.getfixturevalue(member)
...
@pytest.mark.parametrize("member", ["admin_member", "client_member"])
def test_mute_finding_form_view_no_perms(request, member, rf, octopoes_api_connector):
    member = request.getfixturevalue(member)
...
def test_mute_finding_post(rf, redteam_member, octopoes_api_connector, mocker):
    ...
    octopoes_api_connector.get_tree.return_value = ReferenceTree.model_validate(MUTED_FINDING_TREE_DATA)
...
def test_muted_finding_button_not_presence(rf, octopoes_api_connector, network, finding_types, redteam_member):
    octopoes_api_connector.list_findings.return_value = Paginated[Finding](
        count=1,
        items=[
            Finding(
    ...
    octopoes_api_connector.load_objects_bulk.return_value = {
...
def test_muted_finding_button_presence_more_findings_and_post(
    rf, request, member, octopoes_api_connector, network, finding_types
):
    member = request.getfixturevalue(member)
    finding_1 = Finding(
        finding_type=finding_types[0].reference,
    ...
    octopoes_api_connector.list_findings.return_value = Paginated[Finding](count=2, items=[finding_1, finding_2])
...
@pytest.mark.parametrize("member", ["admin_member", "client_member"])
def test_can_mute_findings_perms(rf, request, member, octopoes_api_connector, network, finding_types):
    member = request.getfixturevalue(member)
    octopoes_api_connector.list_findings.return_value = Paginated[Finding](
        count=2,
        items=[
...
def test_findings_list_filtering(rf, request, member, octopoes_api_connector, network, finding_types):
    member = request.getfixturevalue(member)
    # Severity Critical
    finding_1 = Finding(
        finding_type=finding_types[1].reference,
    ...
    assert octopoes_api_connector.list_findings.mock_calls[2].kwargs["severities"] == {RiskLevelSeverity.LOW}
    assert octopoes_api_connector.list_findings.mock_calls[3].kwargs["severities"] == {RiskLevelSeverity.LOW}
