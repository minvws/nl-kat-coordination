...
def test_network_api(drf_client, xtdb):
    ...
    net3 = drf_client.post("/api/v1/objects/network/", data=json.dumps(network), content_type="application/json").json()
...
def test_hostname_api(drf_client, xtdb):
    ...
    hn2 = drf_client.post("/api/v1/objects/hostname/", data=json.dumps(hostname), content_type="application/json").json()
...
def test_bulk_create(drf_client, xtdb):
    ...
    nets = drf_client.post("/api/v1/objects/network/", data=json.dumps(networks), content_type="application/json").json()
    time.sleep(0.3)
    ...
    drf_client.post("/api/v1/objects/hostname/", data=json.dumps(hostnames), content_type="application/json").json()
    time.sleep(0.3)
