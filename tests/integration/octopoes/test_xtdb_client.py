...
from tests.integration.conftest import seed_asset_report, seed_report, seed_system_with_xtdb
...
def test_node_creation_and_deletion(xtdb_http_client: XTDBHTTPClient):
    ...
    assert status.estimateNumKeys is not None
...
def test_query_simple_filter(xtdb_session: XTDBSession, valid_time: datetime):
    ...
    query_string = """{:query {:find [(pull ?3b1ebf3a-3cc1-4e35-8c5f-e8173e55b623 [*])] :where [
    ...
    assert len(xtdb_session.client.query(query_string)) == 1
...
def test_query_for_system_report(
    ...
    seed_system_with_xtdb(xtdb_ooi_repository, xtdb_origin_repository, valid_time)
    ...
    assert {x[0]["Service/primary_key"] for x in result} == {
        "Service|ssh",
        "Service|smtp",
        "Service|https",
        "Service|http",
    ...
    # Queries performed in OpenKAT's system report
    ips = xtdb_octopoes_api_connector.query(
    ...
    for ip in ips:
        ip_services[str(ip.address)] = {
            "hostnames": [
                str(x.name)
                for x in xtdb_octopoes_api_connector.query(
...
def test_query_for_web_system_report(
    ...
    assert (
        len(xtdb_octopoes_api_connector.query(query, valid_time, web_hostname.reference)) == 0
    ...
    resources_finding_types = xtdb_octopoes_api_connector.query(query, valid_time, web_hostname.reference)
    ...
    web_url_finding_types = xtdb_octopoes_api_connector.query(query, valid_time, web_hostname.reference)
    ...
    assert len(xtdb_octopoes_api_connector.query(query, valid_time, web_hostname.reference)) == 0
    assert len(xtdb_octopoes_api_connector.query(query, valid_time, second_web_hostname.reference)) == 1
    ...
    assert len(xtdb_octopoes_api_connector.query(query, valid_time, web_hostname.reference)) == 4
...
def test_query_subclass_fields_and_returning_only_fields(
    ...
    assert xtdb_octopoes_api_connector.query("Network.name", valid_time, "Network|test") == ["test"]
    assert xtdb_octopoes_api_connector.query_many(
        "URL.web_url.netloc.name", valid_time, ["URL|test|https://test.com/security", "URL|test|https://test.com/test"]
    ) == [("URL|test|https://test.com/security", "example.com")]
...
def test_query_children_of_reports(
    ...
    hydrated_report = xtdb_octopoes_api_connector.get_report(report.reference, valid_time)
