...
from octopoes.events.events import OOIDBEvent, OriginDBEvent, ScanProfileDBEvent
...
from tests.integration.conftest import MockEventManager
...
def test_hostname_nxd_ooi(xtdb_octopoes_api_connector: OctopoesAPIConnector, valid_time: datetime):
    ...
    original_size = len(xtdb_octopoes_api_connector.list_origins(valid_time=valid_time))
    assert original_size >= 2
    xtdb_octopoes_api_connector.recalculate_bits()
    bits_size = len(xtdb_octopoes_api_connector.list_origins(valid_time=valid_time))
    ...
    xtdb_octopoes_api_connector.delete(network.reference, valid_time=valid_time)
    xtdb_octopoes_api_connector.delete(hostname.reference, valid_time=valid_time)

    # This sleep is here because otherwise on some systems this test will fail
    # Delete when issue #2083 is resolved...
    time.sleep(2)
    assert len(xtdb_octopoes_api_connector.list_origins(valid_time=valid_time)) < bits_size

    xtdb_octopoes_api_connector.recalculate_bits()

    assert len(xtdb_octopoes_api_connector.list_origins(valid_time=valid_time)) < original_size


def test_events_created_through_crud(
    xtdb_octopoes_service: OctopoesService, event_manager: MockEventManager, valid_time: datetime
):
    network = Network(name="internet")
    ...
    assert isinstance(call1, ScanProfileDBEvent)
    assert isinstance(call2, OOIDBEvent)
    assert isinstance(call3, OriginDBEvent)

    assert call2.old_data is None
    assert call2.new_data == network
    assert call2.operation_type.value == "create"

    assert call3.old_data is None
    assert call3.new_data == origin
    assert call3.operation_type.value == "create"

    xtdb_octopoes_service.ooi_repository.delete_if_exists(network.reference, valid_time)
    xtdb_octopoes_service.commit()

    assert len(event_manager.queue) == 4  # Origin will be deleted by the worker due to the OOI delete event
    call4 = event_manager.queue[3]
    assert isinstance(call4, OOIDBEvent)
    assert call4.operation_type.value == "delete"
...
def test_events_created_in_worker_during_handling(
    ...
    assert len(event_manager.queue) == 4
    event = event_manager.queue[3]  # OOIDelete event
...
def test_affirming_ooi_delete(xtdb_octopoes_api_connector: OctopoesAPIConnector, valid_time: datetime):
    # Make an object A
    network = Network(name="internet")
    xtdb_octopoes_api_connector.save_declaration(Declaration(ooi=network, valid_time=valid_time))
    ...
    xtdb_octopoes_api_connector.save_observation(hostname_origin)
    time.sleep(1)
    assert xtdb_octopoes_api_connector.list_objects({Hostname}, valid_time).count == 1

    # Delete A and validate B is not present
    xtdb_octopoes_api_connector.delete(network.reference, valid_time)
    time.sleep(1)
    assert xtdb_octopoes_api_connector.list_objects({Hostname}, valid_time).count == 0
    ...
    # Affirm object B
    xtdb_octopoes_api_connector.save_affirmation(
...
def test_delecration_ooi_delete(xtdb_octopoes_api_connector: OctopoesAPIConnector, valid_time: datetime):
    # Make an object A
    network = Network(name="internet")
    xtdb_octopoes_api_connector.save_declaration(Declaration(ooi=network, valid_time=valid_time))
    ...
    # Infer object B
    xtdb_octopoes_api_connector.save_declaration(
