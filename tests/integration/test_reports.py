...
def test_web_report(xtdb_octopoes_api_connector: OctopoesAPIConnector, valid_time):
    seed_system(xtdb_octopoes_api_connector, valid_time)

    report = WebSystemReport(xtdb_octopoes_api_connector)
    ...
    xtdb_octopoes_api_connector.save_declaration(Declaration(ooi=finding, valid_time=valid_time))
...
def test_system_report(xtdb_octopoes_api_connector: OctopoesAPIConnector, valid_time):
    seed_system(xtdb_octopoes_api_connector, valid_time)

    report = SystemReport(xtdb_octopoes_api_connector)
    input_ooi = Reference.from_str("Hostname|test|example.com")
    data = report.collect_data([input_ooi], valid_time)[input_ooi]
...
def test_aggregate_report(xtdb_octopoes_api_connector: OctopoesAPIConnector, valid_time, hostname_oois, organization):
    seed_system(xtdb_octopoes_api_connector, valid_time)
    ...
    _, data, _, _ = aggregate_reports(
        xtdb_octopoes_api_connector, hostname_oois, report_ids, valid_time, organization.code
    )
...
def test_multi_report(
    xtdb_octopoes_api_connector: OctopoesAPIConnector,
    xtdb_octopoes_api_connector_2: OctopoesAPIConnector,
    valid_time,
    hostname_oois,
):
    seed_system(xtdb_octopoes_api_connector, valid_time)
    seed_system(xtdb_octopoes_api_connector_2, valid_time)
    time.sleep(2)

    reports = AggregateOrganisationReport.reports["required"] + AggregateOrganisationReport.reports["optional"]
    report_ids = [report_type.id for report_type in reports]
    _, data, _, _ = aggregate_reports(
        xtdb_octopoes_api_connector,
        hostname_oois,
        report_ids,
        valid_time,
        xtdb_octopoes_api_connector.xtdb_session.client.client,
    )
    _, data_2, _, _ = aggregate_reports(
        xtdb_octopoes_api_connector_2,
        hostname_oois,
        report_ids,
        valid_time,
        xtdb_octopoes_api_connector_2.xtdb_session.client.client,
    )
    ...
    report_data_object = ReportData(
        organization_code=xtdb_octopoes_api_connector.xtdb_session.client.client,
        organization_name="Test name",
        organization_tags=["test1"],
        data=data,
    )
    report_data_object_2 = ReportData(
        organization_code=xtdb_octopoes_api_connector_2.xtdb_session.client.client,
    ...
    xtdb_octopoes_api_connector.save_declaration(Declaration(ooi=report_data_object, valid_time=valid_time))
    xtdb_octopoes_api_connector.save_declaration(Declaration(ooi=report_data_object_2, valid_time=valid_time))

    multi_report = MultiOrganizationReport(xtdb_octopoes_api_connector)
    ...
    assert multi_data["organizations"] == [
        xtdb_octopoes_api_connector.xtdb_session.client.client,
        xtdb_octopoes_api_connector_2.xtdb_session.client.client,
    ]
    assert multi_data["tags"] == {"test1": orgs, "test2": [orgs[1]], "test3": [orgs[1]]}
