...
def test_crisis_room_findings_dashboard(rf, mocker, client_member, organization_b, findings_dashboard_mock_data):
    ...
    assertContains(
        response, f'<td><a href="/en/{client_member.organization.code}/">Test Organization</a></td>', html=True
    )
    ...
    assertContains(response, f'<td><a href="/en/{organization_b.code}/">OrganizationB</a></td>', html=True)
...
def test_collect_findings_dashboard(
    dashboard_data, findings_reports, findings_report_bytes_data, findings_dashboard_mock_data, octopoes_api_connector
):
    """
    ...
    """
    octopoes_api_connector.bulk_list_reports.return_value = findings_reports
