...
def test_report_history_less_than_five_subreports_two_input_objects(
    rf, client_member, octopoes_api_connector, mocker, report_list_two_asset_reports
):
    """
    ...
    octopoes_api_connector.list_reports.return_value = Paginated[HydratedReport](
    ...
    assertNotContains(
        response,
        (
            f'<a href="/en/{client_member.organization.code}/reports/report-history/subreports?'
...
def test_report_history_more_than_five_asset_reports_one_input_object(
    rf, client_member, octopoes_api_connector, mocker, report_list_six_asset_reports
):
    """
    ...
    assertContains(
        response,
        f'<a href="/en/{client_member.organization.code}/objects/detail/'
        f'?ooi_id=Hostname%7Cinternet%7Cexample.com">example.com</a>',
        html=True,
    )
    assertContains(response, "Close asset report object details")
...
def test_report_history_asset_reports_table(
    rf, client_member, octopoes_api_connector, mocker, report_list_six_asset_reports, get_asset_reports
):
    mocker.patch("account.mixins.OrganizationView.get_katalogus")
    kwargs = {"organization_code": client_member.organization.code}
    ...
    octopoes_api_connector.query_many.return_value = get_asset_reports
...
def test_report_history_report_type_summary(rf, client_member, octopoes_api_connector, mocker, reports_more_input_oois):
    mocker.patch("account.mixins.OrganizationView.get_katalogus")
