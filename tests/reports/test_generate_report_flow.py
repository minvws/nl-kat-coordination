...
def test_report_types_selection(
    rf, client_member, valid_time, octopoes_api_connector, listed_hostnames, plugins, katalogus_client
):
    """
    Will send the selected report types to the configuration page (set plugins).
    """
    ...
    assert (
        response.headers["Location"] == f"/en/{client_member.organization.code}/reports/generate-report/export-setup/?"
    )


def test_save_generate_report_view(rf, client_member, valid_time, octopoes_api_connector, listed_hostnames, plugins):
    """
...
def test_save_generate_report_view_scheduled(
    rf, client_member, valid_time, octopoes_api_connector, listed_hostnames, plugins
):
    """
