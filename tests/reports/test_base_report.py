...
def test_aggregate_report_select_oois(rf, client_member, octopoes_api_connector, mocker, listed_hostnames):
...
def test_aggregate_report_select_oois_empty_list(rf, client_member, octopoes_api_connector, mocker, listed_hostnames):
    mocker.patch("account.mixins.OrganizationView.get_katalogus")
    kwargs = {"organization_code": client_member.organization.code}
    url = reverse("aggregate_report_select_oois", kwargs=kwargs)
    request = rf.get(url)
    ...
    octopoes_api_connector.list_objects.return_value = Paginated[OOIType](count=len(listed_hostnames), items=[])
...
def test_aggregate_report_choose_report_types(
    rf, client_member, octopoes_api_connector, mocker, listed_hostnames, valid_time
):
    mocker.patch("account.mixins.OrganizationView.get_katalogus")
    kwargs = {"organization_code": client_member.organization.code}
