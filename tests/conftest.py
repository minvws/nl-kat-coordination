...
def create_organization(name, organization_code):
    with patch("openkat.signals.settings.OCTOPOES_FACTORY"), patch("openkat.signals.get_or_create_default_dashboard"):
...
@pytest.fixture
def organization(octopoes_api_connector):
    return create_organization("Test Organization", "org")


@pytest.fixture
def organization_b(octopoes_api_connector):
...
@pytest.fixture
def mock_models_octopoes(octopoes_api_connector):
    return octopoes_api_connector


@pytest.fixture
def tree(octopoes_api_connector, valid_time):
    octopoes_api_connector.get_tree.return_value = ReferenceTree.model_validate(
        {
            "root": {
...
@pytest.fixture()
def dns_records_normalizer(db):
    return Normalizer.objects.create(
        plugin_id="dns-records", static=True, name="DNS records", description="test dns description"
    )


@pytest.fixture()
def katalogus_client(plugins):
...
class MockOctopoesAPIConnector:
    ...
    valid_time: datetime
    tree: dict[Reference, ReferenceTree] = {}
...
@pytest.fixture()
def octopoes_api_connector(mocker, settings):
    with patch("octopoes.core.app.EventManager") as manager:
        manager().client = "testcode"
        mock = mocker.MagicMock()

        def wrapper(organization_code: str):
            return mock

        settings.OCTOPOES_FACTORY = wrapper
...
def get_aggregate_report_from_bytes():
    data = {
        ...
        "health": [
            {"service": "openkat", "healthy": True, "version": "0.0.1.dev1", "additional": None, "results": []},
...
@pytest.fixture
def findings_reports(client_member, client_member_b, raw_a, raw_b):
    bytes_raw_id_a = str(raw_a.id)
    bytes_raw_id_b = str(raw_b.id)
...
def findings_dashboard_mock_data(findings_reports, findings_report_bytes_data):
    ...
    for (recipe_id, report), raw in zip(findings_reports.items(), findings_report_bytes_data):
        findings_dashboard.append({"report": report, "report_data": raw})
