...
from openkat.view_helpers import Breadcrumb, BreadcrumbsMixin, PostRedirect, url_with_querystring
from openkat.views.mixins import ObservedAtMixin, OOIList
from openkat.views.ooi_view import BaseOOIListView, OOIFilterView
...
class BaseReportView(OOIFilterView, ReportBreadcrumbs):
    ...
    def create_report_recipe(
        ...
        create_ooi(api_connector=self.octopoes_api_connector, ooi=report_recipe, observed_at=datetime.now(timezone.utc))
            ...
            for report_type in self.get_report_types():
                for plugin_type in ["required", "optional"]:
                    ...
                    number_of_enabled = sum(
                        (1 if plugin.enabled and plugin.plugin_id in report_type.plugins[plugin_type] else 0)
...
class ViewReportView(ObservedAtMixin, OrganizationView, TemplateView):
    ...
    def get_plugins(self, plugins_dict: dict[str, list[str]]) -> list[dict]:
        plugins: dict[str, list[Boefje | Normalizer]] = {"required": [], "optional": []}
        ...
        return [
            {
                ...
                "enabled": plugin.enabled,  # type: ignore
                "name": plugin.name,
                "scan_level": plugin.scan_level if isinstance(plugin, Boefje) else 0,
                "type": "boefje" if isinstance(plugin, Boefje) else "normalizer",
                "description": plugin.description,
            }
            for required_optional, plugin_details in plugins.items()
