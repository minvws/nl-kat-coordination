    ...
    def pop_item(self, queue: str) -> Optional[QueuePrioritizedItem]:
        ...
        try:
            if WorkerManager.Queue.BOEFJES.value in queue:
                p_item = TypeAdapter(QueuePrioritizedItem).validate_python(self.boefje_responses.pop(0))
            ...
            if WorkerManager.Queue.NORMALIZERS.value in queue:
                p_item = TypeAdapter(QueuePrioritizedItem).validate_python(self.normalizer_responses.pop(0))
                self._popped_items[str(p_item.id)] = p_item
                return p_item
        except IndexError:
