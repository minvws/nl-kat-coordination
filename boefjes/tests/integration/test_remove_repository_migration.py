...
def test_fail_on_non_unique(migration_cd34fdfafdaf):
    ...
    all_plugin_states = [(x[1], x[2], x[3]) for x in session.get_bind().execute(text("SELECT * FROM plugin_state")).fetchall()]
...
def test_downgrade(migration_cd34fdfafdaf):
    ...
    all_plugin_states = [(x[1], x[2], x[3], x[4]) for x in session.get_bind().execute(text("SELECT * FROM plugin_state")).fetchall()]
