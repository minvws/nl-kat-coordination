...
def run(input_ooi: dict, raw: bytes) -> Iterable[NormalizerOutput]:
    ...
    for line in raw.splitlines():
        ...
        # takeover on the same host shares the natural key
        # f"{hostname}|{finding_type}" and all detections collapse into a single
        # Finding. Fall back to the hostname when nuclei reports no usable match
        # location.
