...
from plugin_repository.tests.common import PLUGINS_DIR, load_plugin, upload_plugin


@patch("plugin_repository.api.routers.simplestreams.PLUGINS_DIR", PLUGINS_DIR)
@patch("plugin_repository.utils.index.PLUGINS_DIR", PLUGINS_DIR)
class TestAPI(TestCase):
    ...
    def test_upload_and_list(self):
        ...
        with patch("plugin_repository.api.routers.plugins.PLUGINS_DIR", directory):
            res = upload_plugin(self.client, plugin, b"metadata", b"rootfs")
        self.assertEqual(202, res.status_code)

        # retrieve images index
        with patch(
            "plugin_repository.api.routers.simplestreams.PLUGINS_DIR", directory
        ):
            res = self.client.get("/streams/v1/index.json")
