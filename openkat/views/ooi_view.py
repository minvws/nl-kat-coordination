...
from openkat.forms.base import BaseOpenKATForm, ObservedAtForm
from openkat.forms.ooi_form import _EXCLUDED_OOI_TYPES, ClearanceFilterForm, OOIForm, OrderByObjectTypeForm
from openkat.ooi_helpers import create_ooi
from openkat.paginator import OpenKATPaginator
from openkat.view_helpers import Breadcrumb, BreadcrumbsMixin, get_mandatory_fields, get_ooi_url
from openkat.views.mixins import ConnectorFormMixin, OctopoesView, OOIList, SingleOOIMixin, SingleOOITreeMixin
...
class BaseOOIFormView(SingleOOIMixin, FormView):
    ooi_class: type[OOI]
    form_class: type[BaseOpenKATForm] = OOIForm

    def get_ooi_class(self):
        return self.ooi.__class__ if hasattr(self, "ooi") else None

    def get_form(self, form_class: type[Form] | None = None) -> BaseOpenKATForm:
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            create_ooi(self.octopoes_api_connector, new_ooi, datetime.now(timezone.utc), end_valid_time)
