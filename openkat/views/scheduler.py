...
from openkat.scheduler import SchedulerError, SchedulerTaskNotFound, scheduler_client
from openkat.views.mixins import OctopoesView
...
class SchedulerView(OctopoesView):
    ...
    def setup(self, request, *args, **kwargs):
        ...
        self.scheduler_client = scheduler_client(self.organization)
    ...
    def get_task_details(self, task_id: str) -> Task | None:
        ...
        if not task or task.organization != self.organization:
            raise Http404()

        return task

    def create_report_schedule(self, report_recipe: ReportRecipe, deadline_at: datetime) -> Schedule:
                ...
                return JsonResponse(
                    {
                        ...
                        "valid_time": datetime.fromisoformat(task.data["raw_data"]["boefje_meta"]["ended_at"]).strftime(
                            "%Y-%m-%dT%H:%M:%S"
                        ),
                    },
                    safe=False,
    ...
    def reschedule_task(self, task_id: str) -> None:
        try:
            ...
            if task:
                if task.organization != self.organization:
                    raise SchedulerTaskNotFound()

                new_id = uuid.uuid4()
                task.data["id"] = str(new_id)

                new_task = Task.objects.create(
                    id=new_id, type=task.type, organization=self.organization, data=task.data
                )
