...
from openkat.forms.base import BaseOpenKATForm, CheckboxTable, LabeledCheckboxInput, ObservedAtForm
from openkat.forms.settings import DEPTH_DEFAULT, DEPTH_HELP_TEXT, DEPTH_MAX, SCAN_LEVEL_CHOICES
...
class SelectOOIForm(BaseOpenKATForm):
    ...
    def __init__(self, oois: list[OOI], organization_code: str, mandatory_fields: list | None = None, *args, **kwargs):
        ...
        self.set_choices_for_field("ooi", [self._to_choice(ooi) for ooi in oois])  # TODO: fix "next scan"
        if len(self.fields["ooi"].choices) == 1:
            self.fields["ooi"].initial = self.fields["ooi"].choices[0][0]

    @staticmethod
    def _to_choice(ooi: OOI) -> tuple[str, tuple[OOI, OOI, int, None]]:
        return str(ooi), (ooi, ooi, ooi.scan_profile.level if ooi.scan_profile else 0, None)


class SelectOOIFilterForm(BaseOpenKATForm):
...
class PossibleBoefjesFilterForm(BaseOpenKATForm):
    show_all = forms.NullBooleanField(
