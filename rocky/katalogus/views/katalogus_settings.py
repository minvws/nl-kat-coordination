...
class ConfirmCloneSettingsView(
    ...
    def get_context_data(self, **kwargs):
        ...
        context["to_organization"] = user.organization(code=kwargs["to_organization"])

        return context

    def post(self, request, *args, **kwargs):
        to_organization = user.organization(code=kwargs["to_organization"])
...
class KATalogusSettingsView(OrganizationPermissionRequiredMixin, OrganizationView, FormView):
    ...
    def get_settings(self):
        ...
        katalogus = self.organization.katalogus

        for boefje in katalogus_client.get_boefjes():
            try:
                plugin_setting = katalogus.get_plugin_settings(boefje.id)
            except HTTPError:
                messages.add_message(
                    self.request,
                    messages.ERROR,
