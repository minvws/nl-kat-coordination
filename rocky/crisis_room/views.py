...
def load_findingtype_risks(finding_types: Set[str]) -> Dict[str, str]:
...
class CrisisRoomView(CrisisRoomBreadcrumbsMixin, ConnectorFormMixin, TemplateView):
    ...
    def sort_finding_list_by_total(
        self, org_findingtype_counts: List[OrganizationFindingTypeCount]
    ) -> List[OrganizationFindingTypeCount]:
        is_desc = self.request.GET.get("sort_total_by", "desc") != "asc"
        return sorted(org_findingtype_counts, key=lambda x: x.total, reverse=is_desc)

    def sort_finding_list_by_critical(
        self, org_findingtype_counts: List[OrganizationFindingTypeCount]
    ) -> List[OrganizationFindingTypeCount]:
        is_desc = self.request.GET.get("sort_critical_by", "desc") != "asc"
        return sorted(org_findingtype_counts, key=lambda x: x.total_critical, reverse=is_desc)
    ...
    def get_finding_type_count(self, organization: Organization) -> Dict[str, int]:
        try:
            ...
            return api_connector.get_findingtype_count(valid_time=self.get_observed_at())
    ...
    def get_context_data(self, **kwargs):
        ...
        for org in user.organizations:
            findingtype_counts[(org.code, org.name)] = self.get_finding_type_count(org)

        # find all unique finding types and query their severity
        unique_finding_types = set()
        for org_code, finding_type_count in findingtype_counts.items():
            unique_finding_types.update(finding_type_count.keys())

        severities = load_findingtype_risks(unique_finding_types)

        # aggregate per organization, per severity
        org_findingtype_counts = []
        for (org_code, org_name), finding_type_count in findingtype_counts.items():
            ...
            org_findingtype_counts.append(OrganizationFindingTypeCount(org_name, org_code, org_severity_count))
        ...
        context["org_findingtype_counts"] = self.sort_finding_list_by_total(org_findingtype_counts)
        context["org_findingtype_counts_critical"] = self.sort_finding_list_by_critical(org_findingtype_counts)
