...
class Dashboard(models.Model):
    ...
    recipe = models.CharField(blank=True, max_length=126, unique=True, help_text=_("Recipe ID"))
    ...
    def save(self, *args, **kwargs):
        if not self.recipe and self.organization:
            ...
            recipe = self.create_organization_recipe(valid_time, self.organization)
            self.recipe = recipe.primary_key
            self.create_report_schedule(valid_time, self.organization, recipe)
