...
def is_allowed_action_for_organization(
    action: PageActions, organization: Organization
) -> bool:
...
def get_allowed_actions(organization) -> Dict[str, bool]:
    return {
        action.value: is_allowed_action_for_organization(action, organization)
...
class OrganizationDetailView(
    ...
    def get_organization_members(self) -> List[Dict[str, Any]]:
        ...
        if filters_active:
            organization_members_set = organization_members_set.filter(
                status__in=filters_active
            )
        ...
        organization_members = [
            {
                ...
                "is_in_signal_group": signal_group
                and member.signal_username in signal_group.members,
            }
    ...
    def post(self, request, *args, **kwargs):
        ...
        return redirect(
            reverse("organization_detail", kwargs={"pk": self.kwargs.get("pk")})
        )

    def handle_page_action(self, action):
        if action not in get_allowed_actions(self.object):
            messages.add_message(
                self.request, messages.WARNING, "Action not allowed: " + action
            )
        ...
        try:
            if action == PageActions.SIGNAL_GROUP_CREATE.value:
                ...
                add_member_to_signal_group_for_organization(
                    self.object, member_signal_username
                )
                ...
                send_message_to_signal_group_for_organization(
                    self.object, "This is a test message sent by KAT."
                )
