...
from rocky.views.organization_member_add import MembersUploadView
...
def test_upload_members(rf, superuser_member):
    ...
    assert list(request._messages).pop().message == "Successfully added csv."
    ...
    assert OrganizationMember.objects.filter(organization=superuser_member.organization, status="new").count() == 4
