...
def test_ooi_edit(rf, superuser_member, organization, mock_organization_view_octopoes, network):
    mock_organization_view_octopoes().get.return_value = network

    request = setup_request(rf.get("ooi_edit", {"ooi_id": "Network|testnetwork"}), superuser_member.user)
