...
def test_generate_report_setup_scan_wrong_plugin_id(
    rf, client_member, valid_time, mocker, mock_organization_view_octopoes, listed_hostnames
):
    ...
    request = rf.get(url, {"observed_at": valid_time.strftime("%Y-%m-%d"), "ooi": "all", "report_type": "ipv6-report"})
