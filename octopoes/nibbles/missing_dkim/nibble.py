...
def dkim_query(targets: list[Reference | None]) -> str:
                ...
                :query {{
                    :find [(pull ?var [*])] :where [
                        ...
                        (or-join [?var ?hostname]
                            ...
                            [?var :DKIMExists/hostname ?hostname]
    ...
    if sgn == "100":
        ...
        return pull(
            [
                f"""
                    [?mx :DKIMExists/primary_key "{str(targets[1])}"]
                    [?mx :DKIMExists/hostname ?hostname]
                """
            ]
        )
    elif sgn == "001":
                ...
                :query {{
                    :find [(pull ?var [*])] :where [
                        (or-join [?var]
                            ...
                            [?var :DKIMExists/primary_key "{str(targets[1])}"]
...
NIBBLE = NibbleDefinition(
    ...
    signature=[
        ...
        NibbleParameter(object_type=DKIMExists, parser="[*][?object_type == 'DKIMExists'][]", optional=True),
