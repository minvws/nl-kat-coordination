...
class XTDBSession:
    ...
    def commit(self) -> None:
        ...
        if self.post_commit_callbacks:
            batch = self.post_commit_callbacks
            self.post_commit_callbacks = []

            for callback in batch:
                callback()
            logger.info("Called %s callbacks after committing XTDBSession", len(batch))

            # These callbacks might have triggered a new set of transactions
            self.commit()
