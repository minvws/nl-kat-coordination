...
class OOIRepository(Repository):
    ...
    def load_bulk(self, references: set[Reference] | set[str], valid_time: datetime) -> dict[Reference, OOI]:
        raise NotImplementedError

    def load_bulk_as_list(self, references: set[Reference] | set[str], valid_time: datetime) -> list[OOI]:
        raise NotImplementedError
    ...
    def delete(self, reference: Reference | str, valid_time: datetime) -> None:
        raise NotImplementedError
    ...
    ) -> list[OOI | tuple | dict[Any, Any]] | list[tuple[str, OOIType]]:
        raise NotImplementedError
...
class XTDBOOIRepository(OOIRepository):
    ...
    def __init__(
        self, event_manager: EventManager, session: XTDBSession, scan_profile_repository: ScanProfileRepository
    ):
    ...
    def load_bulk(self, references: set[Reference] | set[str], valid_time: datetime) -> dict[Reference, OOI]:
        ...
        return {ooi.reference: ooi for ooi in oois}

    def load_bulk_as_list(self, references: set[Reference] | set[str], valid_time: datetime) -> list[OOI]:
        if not references:
            return []

        query = Query().where_in(OOI, id=[str(r) for r in references]).pull(OOI, fields="[* {:_reference [*]}]")
    ...
    def save(self, ooi: OOI, valid_time: datetime, end_valid_time: datetime | None = None) -> None:
        ...
        if old_ooi is not None:
            ...
            self.scan_profile_repository.save(None, EmptyScanProfile(reference=ooi.reference), valid_time=valid_time)
