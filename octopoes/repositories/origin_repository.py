...
class OriginRepository(Repository):
    ...
    def list_origins(
        ...
        *,
        ...
        origin_type: str | OriginType | list[OriginType | str] | set[OriginType | str] | None = None,
    ) -> list[Origin]:
        raise NotImplementedError
        ...
        *,
        ...
        if origin_type:
            ...
            if isinstance(origin_type, str):
                where_parameters["origin_type"] = origin_type
            elif isinstance(origin_type, list | set) and all(
                isinstance(otype, OriginType | str) for otype in origin_type
            ):
                where_parameters["origin_type"] = [
                    otype.value if isinstance(otype, OriginType) else otype for otype in origin_type
                ]
