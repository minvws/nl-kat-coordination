...
class FilteringTestCase(unittest.TestCase):
    ...
    def test_apply_filter_related(self):
        filter_request = FilterRequest(
            filters=[
                Filter(column="parent", field="name", operator="eq", value=self.models.get("alice").name),
            ]
        )
    ...
    def test_apply_filter_related_reversed(self):
        filter_request = FilterRequest(
            filters=[
                Filter(column="children", field="name", operator="eq", value=self.models.get("david").name),
            ]
        )
        query = session.query(TestModel)
        filtered_query = apply_filter(TestModel, query, filter_request)
    ...
    def test_apply_filter_related_and_nested__(self):
        filter_request = FilterRequest(
            filters=[
                Filter(column="children", field="data__foo", operator="eq", value="bar"),
            ],
        )
    ...
    def test_apply_filter_related_and_nested_reversed(self):
        filter_request = FilterRequest(
            filters=[
                Filter(column="parent", field="data__foo", operator="eq", value="bar"),
            ],
        )
