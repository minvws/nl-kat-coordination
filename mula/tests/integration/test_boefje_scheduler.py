...
class ScanProfileTestCase(BoefjeSchedulerBaseTestCase):
    ...
    def test_push_tasks_for_scan_profile_mutations(self):
        ...
        ooi = OOIFactory(scan_profile=scan_profile)
        boefje = PluginFactory(scan_level=0, consumes=[ooi.object_type])
        mutation = models.ScanProfileMutation(operation="create", primary_key=ooi.primary_key, value=ooi).json()
    ...
    def test_push_tasks_for_scan_profile_mutations_value_empty(self):
        ...
        # Arrange
        mutation = models.ScanProfileMutation(operation="create", primary_key="123", value=None).json()
    ...
    def test_push_tasks_for_scan_profile_mutations_item_on_queue(self):
        ...
        ooi = OOIFactory(scan_profile=scan_profile)
        boefje = PluginFactory(scan_level=0, consumes=[ooi.object_type])
        mutation1 = models.ScanProfileMutation(operation="create", primary_key=ooi.primary_key, value=ooi).json()
        mutation2 = models.ScanProfileMutation(operation="create", primary_key=ooi.primary_key, value=ooi).json()
