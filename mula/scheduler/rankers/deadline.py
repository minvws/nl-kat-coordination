...
class DefaultDeadlineRanker(Ranker):
    def rank(self, obj: Any) -> int:
        ...
        if obj is not None and obj.schedule is not None and obj.schedule != "":
            try:
                next_run = cron.next_run(obj.cron_expression)
            except Exception as exc:
                raise ValueError(f"Invalid cron expression: {obj.schedule}") from exc
        ...
        jitter_offset = timedelta(seconds=random.uniform(-jitter_range_seconds, jitter_range_seconds))  # noqa
