...
class SchemaStore:
    ...
    def get_schemas(
        ...
        with self.dbconn.session.begin() as session:
            ...
            try:
                count = query.count()
                schema_orm = query.order_by(models.TaskSchemaDB.created_at.desc()).offset(offset).limit(limit).all()
    ...
    def get_schema(self, schema_id: str) -> models.TaskSchema:
        with self.dbconn.session.begin() as session:
            schema_orm = session.query(models.TaskSchemaDB).filter(models.TaskDB.id == schema_id).one_or_none()

            if schema_orm is None:
                raise ValueError(f"TaskSchema not found: {schema_id}")
    ...
    @retry()
    def update_schema(self, schema: models.TaskSchema) -> models.Task:
        with self.dbconn.session.begin() as session:
            schema_orm = session.query(models.TaskSchemaDB).filter(models.TaskDB.id == schema.id).one_or_none()

            if schema_orm is None:
                raise ValueError(f"TaskSchema not found: {schema.id}")

            schema_orm.update(schema.model_dump())
            session.add(schema_orm)

            # TODO: validate cron expression
            updated_schema = models.TaskSchema.model_validate(schema_orm)
