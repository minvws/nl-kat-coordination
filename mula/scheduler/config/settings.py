...
class Settings(BaseSettings):
    ...
    ext_svc_pool_maxsize: int = Field(
        42,
        description="The maximum number of connections to save in the pool for " "a given http host service",
    )

    ext_svc_pool_connections: int = Field(
        42,
        description="The maximum number of connections to allow to a single " "host for a given http host service",
    )
