...
knowledge on a topic, while others ay be more urgent and require less rigor.
...
3.  Create a placeholder RFD file in the `rfd` directory.
    ...
    ```bash
    cp rfd/prototypes/prototype.md rfd/0042-title.md
    ```
...
9.  Merge the Pull Request
    ...
    Discussion can continue on `published` RFD's. Because the discussion link
    in the RFD `discussion:` metadata should be present to the original PR.
    This allows for discussion on the original pull request.
