...
Cypress.Commands.add('login', function ({ user, pass }, navToLogin = true) {
  if (navToLogin) {
    ...
    cy.visit(
      getUrl(Cypress.env('base_url'), '/account/login/')
    );
  }
...
Cypress.Commands.add('isOoiDetailPage', function ({id, title, ooiType}) {
  ...
  cy.get('table thead').contains('Declarations').parents('table').contains(ooiType);
});

Cypress.Commands.add('switchOnBoefje', function(boefjeName) {
  cy.navTo('katalogus');
  cy.get('.plugins h1').contains(boefjeName)
...
Cypress.Commands.add('navToHome', function() {
    cy.get('header nav[data-media="(min-width: 56rem)"] > ul > li:first-child').click();
});
