...
from openkat.forms.boefje import BoefjeSetupForm
...
class BoefjeSetupView(OrganizationPermissionRequiredMixin, OrganizationView, FormView):
    ...
    permission_required = "openkat.can_add_boefje"
...
class AddBoefjeView(BoefjeSetupView):
    ...
    def form_valid(self, form):
        create_boefje_with_form_data(form.cleaned_data, self.plugin_id, self.created)
...
class AddBoefjeVariantView(BoefjeSetupView):
    ...
    def get_initial(self):
        ...
        initial["boefje_schema"] = self.plugin.schema
        initial["consumes"] = self.plugin.consumes
...
class EditBoefjeView(BoefjeSetupView):
    ...
    def form_valid(self, form):
        edit_boefje_with_form_data(form.cleaned_data, self.plugin_id)
...
def create_boefje_with_form_data(form_data, plugin_id: str, created: datetime | None):
    ...
    return Boefje(
        ...
        produces=produces,
        schema=form_data["boefje_schema"],
        interval=interval,
        run_on=run_on,
...
def edit_boefje_with_form_data(form_data, plugin: Boefje):
    plugin.arguments = [] if not form_data["oci_arguments"] else form_data["oci_arguments"].split()
    plugin.consumes = (
        [] if not form_data["consumes"] else form_data["consumes"].strip("[]").replace("'", "").split(", ")
    )
    produces = [] if not form_data["produces"] else form_data["produces"].split(",")
    plugin.produces = {p.strip() for p in produces}
    plugin.interval = None
    plugin.run_on = None

    if form_data["scan_type"] == "interval" and form_data.get("interval_number"):
        plugin.interval = get_interval_minutes(int(form_data["interval_number"]), form_data["interval_frequency"])
    elif form_data["scan_type"] == "run_on":
        plugin.run_on = form_data["run_on"].split("-")

    input_objects = set()

    for input_object in plugin.consumes:
        input_objects.add(type_by_name(input_object))

    plugin.consumes = input_objects
    plugin.save()
