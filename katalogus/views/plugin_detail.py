...
from katalogus.views.mixins import SingleNormalizerView, SinglePluginView
from katalogus.views.plugin_settings_list import PluginSettingsListView
from octopoes.models import ScanLevel
from octopoes.models.types import OOIType
from openkat.forms.ooi import SelectOOIFilterForm, SelectOOIForm
...
class PluginDetailView(TaskListView, SinglePluginView):
        ...
        if self.action == self.SCAN_OOIS:
            ...
            if selected_oois and self.plugin.id:
                ...
                if oois_with_clearance_level:
                    self.run_boefje_for_oois(boefje=self.plugin, oois=oois_with_clearance_level)

                if oois_without_clearance_level:
                    if not self.organization_member.has_perm("openkat.can_set_clearance_level"):
                        messages.error(
                            request,
                            _(
    ...
    def get_queryset(self):
        return super().get_queryset().filter(**{f"data__{self.task_type}__id": self.plugin.id})
    ...
    def get_oois(self, selected_oois: list[str]) -> dict[str, Any]:
        ...
        for ooi in selected_oois:
            ...
            if ooi_object.scan_profile and ooi_object.scan_profile.level >= self.plugin.scan_level:
...
class NormalizerDetailView(PluginDetailView, SingleNormalizerView):
...
class BoefjeDetailView(PluginDetailView, PluginSettingsListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["variants"] = self.get_katalogus().get_plugins(plugin_type="boefje", oci_image=self.plugin.oci_image)
    ...
    def get_form_consumable_oois(self) -> list[OOIType]:
    ...
    def get_form_filtered_consumable_oois(self) -> list[OOIType]:
        ...
        return self.octopoes_api_connector.list_objects(
            ...
            scan_level={level for level in ScanLevel if level.value >= self.plugin.scan_level},
        ).items
