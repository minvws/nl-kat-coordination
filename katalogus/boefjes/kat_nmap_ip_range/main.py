...
def run(input_ooi: dict, boefje: dict) -> list[tuple[set, bytes | str]]:
    ...
    ip_range = ip_network(f"{input_ooi['start_ip']['address']}/{str(input_ooi['mask'])}")
    ...
    if ip_range.prefixlen < min_mask:
        logging.info("Minimum expected VLSM %d > %d, skipping this range.", min_mask, ip_range.prefixlen)
