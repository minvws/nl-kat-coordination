...
class LocalBoefjeHandler(BoefjeHandler):
    ...
    def handle(self, task: Task) -> tuple[dict, BoefjeOutput]:
        ...
        logger.info("Starting boefje %s[%s]", boefje_meta.boefje.plugin_id, str(boefje_meta.id))
        ...
        try:
            ...
            try:
                ...
                boefje_resource = self.local_repository.by_id(boefje_meta.boefje.plugin_id)
            except KeyError:
            ...
            if not isinstance(boefje_resource, BoefjeResource):
                raise JobRuntimeError(f"Not a boefje: {boefje_meta.boefje.plugin_id}")
            ...
            with TemporaryEnvironment() as temporary_environment:
                ...
                boefje_results = boefje_resource.module().run(
                    boefje_meta.input_ooi_data, boefje_meta.boefje.model_dump(mode="json")
                )  # TODO: figure out a merge between using boefje_meta and input_ooi dicts
                boefje_meta.ended_at = datetime.now(timezone.utc)
        except BaseException as e:
            logger.exception("Error running boefje %s[%s]", boefje_meta.boefje.plugin_id, str(boefje_meta.id))
            boefje_meta.ended_at = datetime.now(timezone.utc)
            boefje_results = [({"error/boefje"}, traceback.format_exc())]
            error = e

        logger.info("Saving to Bytes for boefje %s[%s]", boefje_meta.boefje.plugin_id, str(boefje_meta.id))

        if not boefje_results:
            logger.info("No results for boefje %s[%s]", boefje_meta.boefje.plugin_id, boefje_meta.id)
            return {}, BoefjeOutput(status=StatusEnum.COMPLETED, files=[])
        ...
        logger.info(
            "Saved %s raw files for boefje %s[%s]", len(raw_file_ids), boefje_meta.boefje.plugin_id, boefje_meta.id
        )

        if error is not None:
            raise error

        return raw_file_ids, boefje_output
