...
BOEFJES_DIR = Path(__file__).parent.parent / "boefjes"
...
class BoefjeResource:
    ...
    def module(self):
        if (self.path / ENTRYPOINT_BOEFJES).exists():
            return get_runnable_module_from_package(self.package, ENTRYPOINT_BOEFJES, parameter_count=2)
...
class NormalizerResource:
    ...
    def module(self):
        return get_runnable_module_from_package(self.package, ENTRYPOINT_NORMALIZERS, parameter_count=2)
...
def _cached_resolve_boefjes(path: Path) -> dict[str, BoefjeResource]:
    ...
    for path, package in paths_and_packages:
        try:
            boefje_resources.append(BoefjeResource(path, package, hash_path(path)))
        except ModuleException as exc:
            logger.exception(exc)

    return {resource.boefje.plugin_id: resource for resource in boefje_resources}
...
def _cached_resolve_normalizers(path: Path) -> dict[str, NormalizerResource]:
    ...
    return {resource.normalizer.plugin_id: resource for resource in normalizer_resources}
...
def _default_mime_types(boefje: Boefje) -> set:
    mime_types = {f"boefje/{boefje.plugin_id}"}

    if boefje.version is not None:
        mime_types = mime_types.union({f"boefje/{boefje.plugin_id}-{boefje.version}"})
