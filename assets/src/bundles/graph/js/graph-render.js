...
const loopTree = (parent, obj, callback) => {
  ...
  const childrenObjects = Object.values(obj)
    .filter(el => el && typeof el == 'object');

  childrenObjects.forEach(child => loopTree(obj, child, callback));
};

const defaultStyles = [ // the default styles for the graph
  {
    selector: 'node',
    style: {
...
const getGraphStyles = () => ooiTypes.reduce((styles, ooiType) => {
  return [
    ...styles,
    {
...
const goToOoi = (ooiId) => {
  ...
  window.location.href = window.location.href.replace(rootObj['crux.db/id'], ooiId);
};
