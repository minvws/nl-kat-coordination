...
```shell
docker compose --env-file .env-release --env-file .env -f docker-compose.release-example.yml up -d
```
...
```shell
docker compose --env-file .env-release --env-file .env -f docker-compose.release-example.yml exec katalogus python3 -m boefjes.seed
```
...
```shell
docker compose --env-file .env-release --env-file .env -f docker-compose.release-example.yml exec rocky python3 manage.py loaddata OOI_database_seed.json
```
...
```shell
docker compose --env-file .env-release --env-file .env -f docker-compose.release-example.yml exec rocky python3 manage.py createsuperuser
```
...
```shell
docker compose --env-file .env-release --env-file .env -f docker-compose.release-example.yml exec rocky python3 manage.py setup_dev_account
```
