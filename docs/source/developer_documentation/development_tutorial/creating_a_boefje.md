...
For ease of development, we added a generic CVE normalizer. When we just want to create a CVE finding on the input OOI, we can return the CVE ID as a string with `cve/id` as mime-type.
